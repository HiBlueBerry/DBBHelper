...
float TiltMask(float2 uv,float offset,float area,float spread)
{
    float centerY=uv.y*2.0-1.0+offset;//[0,1]->[-1,1]
    return pow(abs(centerY*area),spread);
}
float stride=0.02;//控制Kawase模糊(对角线采样)的采样步长
float spread=1.0;//控制清晰的长条区域的发散程度，应该大于0
float offset=0.0;//控制长条区域的中心位置，0.0为长条区域在中心
float area=1.0;//控制清晰的长条区域的面积，取值为0到1
int mask_mode=0;//掩码模式，此模式方便调试，掩码的黑色区域即为清晰的长条区域
int reverse=0;//掩码的黑色区域反转
float4 main(pInput pin):SV_TARGET
{
    ...
    float proption=clamp(TiltMask(pin.UV,offset,area,spread),0.0,1.0);
    proption=lerp(proption,1.0-proption,reverse);
