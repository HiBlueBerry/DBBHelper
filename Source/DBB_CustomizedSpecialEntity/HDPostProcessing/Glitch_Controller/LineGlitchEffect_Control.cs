...
{
    [CustomEntity("DBBHelper/LineGlitchEffectControl")]
    public class LineGlitchEffectControl : Entity
    {
        //------------------与控制特效相关------------------
        ...
        private float num_level_start = 1.0f;
        private float num_level_end = 1.0f;
        private string num_level_control_mode = "Linear";
        private float num_level_tmp = 1.0f;

        private float detail_start = 1.0f;
        private float detail_end = 1.0f;
        private string detail_control_mode = "Linear";
        private float detail_tmp = 1.0f;
        ...
        public LineGlitchEffectControl(EntityData data, Vector2 offset)
        {
            ...
            num_level_start = data.Float("NumLevelStart");
            num_level_end = data.Float("NumLevelEnd");
            num_level_control_mode = data.Attr("NumLevelControlMode");

            detail_start = data.Float("DetailStart");
            detail_end = data.Float("DetailEnd");
            detail_control_mode = data.Attr("DetailControlMode");
        ...
        private void UpdateParameter()
        {
            ...
            if (x_proportion >= 0.0f && x_proportion <= 1.0f && y_proportion >= 0.0f && y_proportion <= 1.0f)
            {
                ...
                if (area_control_mode == "Left_to_Right")
                {
                    ...
                    num_level_tmp = Calc_tmpValue(x_proportion, num_level_control_mode, num_level_start, num_level_end);
                    detail_tmp = Calc_tmpValue(x_proportion, detail_control_mode, detail_start, detail_end);
                ...
                else if (area_control_mode == "Bottom_to_Top")
                {
                    ...
                    num_level_tmp = Calc_tmpValue(y_proportion, num_level_control_mode, num_level_start, num_level_end);
                    detail_tmp = Calc_tmpValue(y_proportion, detail_control_mode, detail_start, detail_end);
                ...
                else if (area_control_mode == "Instant")
                {
                    ...
                    num_level_tmp = num_level_end;
                    detail_tmp = detail_end;
                    strength_tmp = strength_end;
                }
                List<Entity> LineGlitchEffectList;
                if (!DBBCustomEntityManager.TrackEntityOf_SubType(typeof(LineGlitchEffect), out LineGlitchEffectList) || LineGlitchEffectList.Count == 0)
                {
                    return;
                }
                foreach (var entity_item in LineGlitchEffectList)
                {
                    var item = entity_item as LineGlitchEffect;
                    if (item.label == label && item.Active == true)
                    {
                        item.velocity = velocity_tmp;
                        item.num_level = num_level_tmp;
                        item.detail = detail_tmp;
