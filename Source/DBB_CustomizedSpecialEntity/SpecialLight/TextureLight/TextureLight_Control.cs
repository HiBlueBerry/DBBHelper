...
{
    [CustomEntity("DBBHelper/TextureLightControl")]
    public class TextureLightControl : Entity
    {
        //------------------与控制特效相关------------------
        ...
        public TextureLightControl(EntityData data, Vector2 offset)
        {
        ...
        private void UpdateParameter()
        {
            ...
            if (x_proportion >= 0.0f && x_proportion <= 1.0f && y_proportion >= 0.0f && y_proportion <= 1.0f)
            {
                ...
                List<Entity> TextureLightList;
                if (!DBBCustomEntityManager.TrackEntityOf_SubType(typeof(TextureLight), out TextureLightList) || TextureLightList.Count == 0)
                {
                    return;
                }
                foreach (var entity_item in TextureLightList)
                {
                    var item = entity_item as TextureLight;
